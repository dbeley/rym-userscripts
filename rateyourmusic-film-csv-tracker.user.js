...
// @version      1.2.0
// @description  Capture film metadata on RateYourMusic pages and keep a CSV in sync (auto-save or manual download). Provides cross-domain API access.
// @author       dbeley
...
(function () {
  ...
  async function copyDataAsJson() {
    ...
    if (typeof GM_setClipboard === "function") {
      ...
      alert(
        `Copied ${Object.keys(records).length} film records to clipboard as JSON.\n\n` +
        `You can now paste this data into another script or application.`
      );
      console.info("[rateyourmusic-film-csv] JSON data copied to clipboard");
    } else {
      ...
      try {
        ...
        console.info("[rateyourmusic-film-csv] JSON data copied to clipboard (fallback)");
      } catch (err) {
        alert("Failed to copy to clipboard. Check console for data.");
        console.log("[rateyourmusic-film-csv] JSON data:", json);
      } finally {
  ...
  function exposeDataApi() {
    ...
    if (typeof window !== "undefined") {
      window.RYMFilmCsvTracker = {
        ...
        // Search records by name, director, or genre
        ...
        async searchRecords(query, limit = 100) {
          ...
          const results = Object.values(records).filter(record => 
            ...
            (record.directors && record.directors.toLowerCase().includes(lowerQuery)) ||
            (record.primaryGenres && record.primaryGenres.toLowerCase().includes(lowerQuery)) ||
            (record.secondaryGenres && record.secondaryGenres.toLowerCase().includes(lowerQuery))
          );
      ...
      console.info("[rateyourmusic-film-csv] API exposed at window.RYMFilmCsvTracker");
    }
  }
})();
