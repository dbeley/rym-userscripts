...
(function() {
    ...
    function getChartInfo() {
        ...
        const pathParts = path.split('/').filter(p => p && !['charts', 'top', 'popular', 'esoteric', 'diverse', 'song'].includes(p) && !/^\d{4}$/.test(p));
    ...
    function handleDownloadCSVClick() {
        ...
        const filename = `rym_song_charts_${chartPart}_${timestamp}.csv`;
    ...
    function handleDownloadPlainTextClick() {
        ...
        // Generate filename with timestamp
        const filename = `rym_song_charts_${chartPart}_${timestamp}.txt`;
