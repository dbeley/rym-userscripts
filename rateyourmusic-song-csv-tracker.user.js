...
// @version      1.1.0
// @description  Capture song metadata on RateYourMusic song and chart pages and keep a CSV in sync (auto-save or manual download). Provides cross-domain API access.
// @author       dbeley
...
(function () {
  ...
  GM_registerMenuCommand("Copy song data as JSON", () => {
  ...
  async function copyDataAsJson() {
    ...
    if (typeof GM_setClipboard === "function") {
      ...
      alert(
        `Copied ${Object.keys(records).length} song records to clipboard as JSON.\n\n` +
        `You can now paste this data into another script or application.`
      );
      console.info("[rateyourmusic-song-csv] JSON data copied to clipboard");
    } else {
      ...
      try {
        ...
        console.info("[rateyourmusic-song-csv] JSON data copied to clipboard (fallback)");
      } catch (err) {
        alert("Failed to copy to clipboard. Check console for data.");
        console.log("[rateyourmusic-song-csv] JSON data:", json);
      } finally {
  ...
  function exposeDataApi() {
    ...
    if (typeof window !== "undefined") {
      ...
      console.info("[rateyourmusic-song-csv] API exposed at window.RYMSongCsvTracker");
    }
  }
})();
